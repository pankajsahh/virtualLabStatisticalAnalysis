...
const Simulation=()=>{
    ...
    const handleSubmit =(e)=>{
        e.preventDefault()
        axios.post('http://localhost:4000/',{code:val,args:args})
    ...
    return(
        <>
            ...
            <div style={{display:'flex'}}>
                ...
                <textarea id='code' style={{height:'80vh',width:'45vw'}} onChange={handleChange} value={val}/>
                <input id='args' onChange={handleChange} placeholder={'enter comma seperated input(s)'} />
                <input type='submit' />
                </form>
                <div style={{backgroundColor:'black',color:'green',height:'80vh',width:'40vw',whiteSpace:'pre-wrap'}}>
                    {res}
                </div>
            </div>
        </>
